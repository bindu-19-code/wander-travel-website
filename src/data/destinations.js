const destinations = [
  {
    id: 1,
    name: "Paris",
    country: "France",
    region: "Europe",
    description:
      "A timeless city of art, architecture, cafés and unforgettable evenings.",
    image:
      "https://images.pexels.com/photos/532826/pexels-photo-532826.jpeg",
    places: [
      {
        name: "Eiffel Tower",
        description:
          "Paris's most iconic landmark, offering beautiful views across the city.",
        image:
          "https://images.pexels.com/photos/699466/pexels-photo-699466.jpeg",
      },
      {
        name: "Louvre Museum",
        description:
          "A world-famous museum home to an extraordinary collection of art.",
        image:
          "https://images.pexels.com/photos/2363/france-landmark-louvre-louvre-museum.jpg",
      },
      {
        name: "Montmartre",
        description:
          "A charming historic neighborhood known for its artistic atmosphere.",
        image:
          "https://images.pexels.com/photos/338515/pexels-photo-338515.jpeg",
      },
    ],
  },

  {
    id: 2,
    name: "Tokyo",
    country: "Japan",
    region: "Asia",
    description:
      "A fascinating mix of ancient traditions, futuristic streets and incredible food.",
    image:
      "https://images.pexels.com/photos/2506923/pexels-photo-2506923.jpeg",
    places: [
      {
        name: "Shibuya Crossing",
        description:
          "One of the world's busiest and most recognizable pedestrian crossings.",
        image:
          "https://images.pexels.com/photos/2506923/pexels-photo-2506923.jpeg",
      },
      {
        name: "Senso-ji Temple",
        description:
          "Tokyo's oldest temple and one of the city's most visited landmarks.",
        image:
          "https://images.pexels.com/photos/402028/pexels-photo-402028.jpeg",
      },
      {
        name: "Tokyo Skytree",
        description:
          "A spectacular observation tower with panoramic views over Tokyo.",
        image:
          "https://images.pexels.com/photos/2614818/pexels-photo-2614818.jpeg",
      },
    ],
  },

  {
    id: 3,
    name: "Bali",
    country: "Indonesia",
    region: "Asia",
    description:
      "Tropical landscapes, peaceful temples, beaches and unforgettable sunsets.",
    image:
      "https://images.pexels.com/photos/1538177/pexels-photo-1538177.jpeg",
    places: [
      {
        name: "Uluwatu Temple",
        description:
          "A dramatic sea temple perched high above Bali's southern coastline.",
        image:
          "https://images.pexels.com/photos/2166553/pexels-photo-2166553.jpeg",
      },
      {
        name: "Tegallalang Rice Terraces",
        description:
          "Beautiful layered rice fields surrounded by lush tropical scenery.",
        image:
          "https://images.pexels.com/photos/2166553/pexels-photo-2166553.jpeg",
      },
      {
        name: "Seminyak Beach",
        description:
          "A relaxed coastal destination known for sunsets, cafés and beach life.",
        image:
          "https://images.pexels.com/photos/1450353/pexels-photo-1450353.jpeg",
      },
    ],
  },

  {
    id: 4,
    name: "Dubai",
    country: "United Arab Emirates",
    region: "Middle East",
    description:
      "A bold city of futuristic architecture, luxury experiences and desert adventures.",
    image:
      "https://images.pexels.com/photos/1470502/pexels-photo-1470502.jpeg",
    places: [
      {
        name: "Burj Khalifa",
        description:
          "The world's tallest building with extraordinary views of Dubai.",
        image:
          "https://images.pexels.com/photos/3787839/pexels-photo-3787839.jpeg",
      },
      {
        name: "Palm Jumeirah",
        description:
          "A spectacular man-made island shaped like a palm tree.",
        image:
          "https://images.pexels.com/photos/1470502/pexels-photo-1470502.jpeg",
      },
      {
        name: "Dubai Desert",
        description:
          "Experience golden dunes, sunsets and traditional desert adventures.",
        image:
          "https://images.pexels.com/photos/1001435/pexels-photo-1001435.jpeg",
      },
    ],
  },

  {
    id: 5,
    name: "Rome",
    country: "Italy",
    region: "Europe",
    description:
      "Ancient history, remarkable architecture and some of the world's best food.",
    image:
      "https://images.pexels.com/photos/1797161/pexels-photo-1797161.jpeg",
    places: [
      {
        name: "Colosseum",
        description:
          "An ancient Roman amphitheatre and one of the world's most famous landmarks.",
        image:
          "https://images.pexels.com/photos/1797161/pexels-photo-1797161.jpeg",
      },
      {
        name: "Trevi Fountain",
        description:
          "A magnificent Baroque fountain and one of Rome's most iconic sights.",
        image:
          "https://images.pexels.com/photos/161853/trevi-fountain-rome-italy-monument-161853.jpeg",
      },
      {
        name: "Pantheon",
        description:
          "A remarkably preserved ancient Roman building with a famous domed roof.",
        image:
          "https://images.pexels.com/photos/2064827/pexels-photo-2064827.jpeg",
      },
    ],
  },

  {
    id: 6,
    name: "New York",
    country: "United States",
    region: "Americas",
    description:
      "An energetic city of skyscrapers, culture, food and endless possibilities.",
    image:
      "https://images.pexels.com/photos/466685/pexels-photo-466685.jpeg",
    places: [
      {
        name: "Times Square",
        description:
          "The bright, energetic heart of Midtown Manhattan.",
        image:
          "https://images.pexels.com/photos/290386/pexels-photo-290386.jpeg",
      },
      {
        name: "Central Park",
        description:
          "A huge urban park offering a peaceful escape in the middle of Manhattan.",
        image:
          "https://images.pexels.com/photos/2901209/pexels-photo-2901209.jpeg",
      },
      {
        name: "Statue of Liberty",
        description:
          "An iconic symbol of New York and a landmark of the United States.",
        image:
          "https://images.pexels.com/photos/40122/statue-of-liberty-new-york-ny-usa-40122.jpeg",
      },
    ],
  },
];

export default destinations;